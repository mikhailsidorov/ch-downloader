...
'''
    ...
    def load_course(self, response):
        ...
        course_loader.add_css('original_name', 'article header div.original-name::text')
        course_loader.add_css('description', 'article div.standard-block p::text')
        course_loader.add_css('materials', 'article div.standard-block a.downloads::attr(href)')
        duration = response.css('article div.standard-block__duration::text').extract_first().split(' ')[1]
    ...
    def load_lesson(self, selector):
        ...
        url = selector.xpath('.//link[@itemprop="contentUrl"]/@href').extract_first()
